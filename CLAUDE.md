...
- `server/utils/` — Shared helpers, seeds, filters:
  - `helpers.js` — `logAuditEvent()`, `logUnauthorizedPartidasAccess()`, `computeSaldo()`, `isPartidaMilKey()`, `ensureAuditoriaEventosSchema()`
...
- `server/sql/` — SQL migration scripts (applied idempotently at startup)

**Request flow:** Frontend (`public/`) calls `GET/POST /api/*` → `server.js` applies auth middleware → route handlers → PostgreSQL via `db.js`.

**Database access:** `db.js` exports `query(text, params)` for parameterized queries and `getClient()` for transaction clients (manual `BEGIN/COMMIT/ROLLBACK`). Both read `DATABASE_URL` from `.env`.

**Frontend:** Served as static files from Express. Each HTML page (`suficiencia_presupuestal.html`, `comprometido.html`, etc.) has a matching JS file in `public/js/`. Shared modules:
- `script.js` — session/localStorage helpers, shared fetch utilities
- `navbar.js` — dynamic navbar injected into every page
- `candados.fuentes.js` — `getFuentesPermitidas()` / `getPartidasPermitidas()` client-side permission checks
- `candados.proyectos.js` — `DG_DA_PROYECTOS_FILTERS` static map of allowed projects per DG/DA
- `alerts.js` — SweetAlert2 wrappers
- `session-guard.js` — redirects to login if no valid session

Session state stored in `localStorage` keys `cp_usuario` and `cp_app_data_v1`.

**File generation:** `pdf-lib` (PDF creation) and `xlsx`/`SheetJS` (Excel export) are used in route handlers for document download endpoints.
...
**`isUserL00117(req)`:** Each route file that needs it defines its own local async function (not centralized) that queries DB for the user's DG/DA claves. Same pattern for `isUserE00(req)`.
...
| `/api/admin/usuarios` | `admin-usuarios.routes.js` | GOD only |
...
- **Partidas mil (1xxx):** Only visible to DG L00 with DA 117 and DG E00. Detected by `isPartidaMilKey()` in `helpers.js`. Unauthorized access attempts are logged to `auditoria_accesos` via `logUnauthorizedPartidasAccess()`.
- **IVA:** Does not apply to partidas mil in suficiencias; other partidas keep IVA.
- **IEPS/Pensiones fields:** Hidden (zeroed/nulled) for users other than L00/117 and E00 via `sanitizeFinancialFieldsForLimitedView()` in `financial-fields-perm.js`.
- **Reconducciones:** Restricted to Monday–Thursday, with an exception for L00/117.
- **Saldo formula:** `presupuesto − total_gastado + total_reconducido` (via `computeSaldo()` in `helpers.js`). The DB function `fn_saldo_disponible_partida()` avoids double-counting across suficiencia → comprometido → devengado phases.
- **Folio numbering:** `ECA-YYYY-MM-SP-NNNN` for suficiencias (generated by DB trigger `fn_set_no_suficiencia`); `ECA-YYYY-MM-CP-NNNN` and `ECA-YYYY-MM-DEV-NNNN` generated in JS routes using `process.env.MUNICIPIO_PREFIJO_FOLIO || "ECA"`.

## Audit System

Two audit tables, both created on-demand (idempotent DDL in `helpers.js`):
- `auditoria_eventos` — general audit log; auto-populated for all write methods via `res.on("finish")` in `authRequired`. Contains `tipo`, `entidad`, `entidad_id`, `estado`, `detalles` (JSONB with `trace_id`).
- `auditoria_accesos` — unauthorized access attempts to partidas mil specifically.
...
On start, `server.js` runs the following migrations idempotently (each checks if already applied via `to_regclass()`, `information_schema.columns`, or `pg_indexes`), then seeds:
