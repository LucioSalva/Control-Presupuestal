...
(() => {
  ...
  async function cargarSuficienciaEnFormulario(id) {
    ...
    // Guardar fuente del registro cargado para usarla en PDF
    {
      const fId = String(data.id_fuente ?? "").trim();
      const fInfo = fuentesMap[fId] || fuentesRows.find((r) => String(r.id ?? "").trim() === fId);
      _currentFuente = {
        id: fId,
  ...
  async function save() {
    ...
    if (!esUsuarioL00117) {
      ...
      // Guardar fuente de la primera suficiencia creada (para PDF)
      _currentFuente = {
        id: "",
  ...
  async function generarPDF() {
    ...
    // Normalizar folio temprano para usarlo en campo PDF y nombre de archivo
    const folioRaw = String(payload.folio_num || "").trim();
    const folio = (() => {
      if (!folioRaw) return "SUFICIENCIA";
      if (/^ECA-\d{4}-\d{2}-SP-\d{4}$/.test(folioRaw)) return folioRaw;
      ...
      if (onlyDigits) {
        ...
        return `ECA-${year}-${month}-SP-${onlyDigits.padStart(4, "0")}`;
      }
      return "SUFICIENCIA";
    })();
    ...
    try {
      ...
      console.log("[SP][PDF] Campos del formulario PDF (" + allFields.length + " campos):");
      allFields.forEach((f) => {
        try { console.log("  →", JSON.stringify(f.getName()), "tipo:", f.constructor?.name); } catch {}
      });
    } catch (e) {
      console.warn("[SP][PDF] No se pudieron listar campos:", e?.message);
    }
    ...
    // Fuente: primary = _currentFuente (guardado al crear/cargar suficiencia)
    // Fallback = [name="fuente"] SELECT (no existe en este formulario multi-fuente,
    //            pero se mantiene para compatibilidad futura)
    let fuenteClave = String(_currentFuente?.clave || "").trim();
    let fuenteDesc = String(_currentFuente?.nombre || "").trim();
    if (!fuenteClave) {
      ...
      fuenteClave = String(fuenteInfo?.clave || "").trim();
      fuenteDesc = String(fuenteInfo?.fuente || "").trim();
    ...
    console.log("[SP][PDF] fuente →", { clave: fuenteClave, desc: fuenteDesc, _currentFuente });
    ...
    // no impida que estos tres se llenen.
    {
      ...
      if (!folioEscrito) {
        ...
        for (const v of variantesNoSuf) {
          setTextSafe(v, folio, sizeFolios, PDFLib?.TextAlignment?.Center);
          if (touchedFields.has(v)) { folioEscrito = true; break; }
        }
      }
      // Intentar por patrón (campo que contenga "suficiencia" — excluye "No" solo)
      if (!folioEscrito) {
        const patternWrote = setTextByPattern(["suficiencia"], folio, sizeFolios, PDFLib?.TextAlignment?.Center);
        if (patternWrote) folioEscrito = true;
      }
      // Fallback: dibujar texto directamente en la página en la esquina superior derecha
      if (!folioEscrito) {
        try {
          const page = pdfDoc.getPages()[0];
          const { width, height } = page.getSize();
          const font = await pdfDoc.embedFont(PDFLib.StandardFonts.Helvetica);
          const textWidth = font.widthOfTextAtSize(folio, sizeFolios);
          page.drawText(folio, {
          ...
          console.warn("[SP][PDF] No se pudo dibujar el folio:", eDraw?.message);
        }
      }
      console.log("[SP][PDF] No. de Suficiencia →", JSON.stringify(folio), "| escrito:", folioEscrito);
      ...
      console.log("[SP][PDF] FUENTE DE FINANCIAMIENTO →", JSON.stringify(fuenteClave), "| en touchedFields:", touchedFields.has("FUENTE DE FINANCIAMIENTO"));

      // NOMBRE F.F
      setTextSafe("NOMBRE F.F", fuenteDesc || "", sizeDG, PDFLib?.TextAlignment?.Center);
      setTextByPattern(["nombre", "f.f"], fuenteDesc || "", sizeDG, PDFLib?.TextAlignment?.Center);
      ...
      for (const f of form.getFields()) {
        try {
          ...
          for (const w of widgets) {
            try {
              // Borrar AP existente → se regenerará limpio
              w.dict.delete(PDFName.of("AP"));
              // Borrar MK completo → sin MK no hay color de fondo en el AP nuevo
    ...
    // Flatten: eliminar widgets huérfanos antes para evitar el error
    // "Could not find page for PDFRef" (bug conocido de pdf-lib con ciertos templates)
    try {
            ...
            for (const w of widgets) {
              try {
                ...
                if (pageRef && !validPageRefs.has(pageRef.toString())) {
                  // Widget apunta a página inexistente — remover su referencia de página
